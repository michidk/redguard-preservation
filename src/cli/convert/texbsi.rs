...
pub(super) fn handle_texbsi_convert(args: &ConvertArgs, output_path: &Path) -> Result<()> {
    ...
    for image in &bsi_file.images {
        ...
        if args.all_frames && image.frame_count > 1 {
        ...
        image_metadata.push(json!(entry));
