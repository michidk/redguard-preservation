...
impl PvoHeader {
    ...
    pub fn interior_nodes(&self) -> u32 {
...
impl OctrNode {
    ...
    pub fn record_size(&self) -> usize {
        5 + self.child_mask.count_ones() as usize * 4
    }
}
...
impl PvoFile {
    ...
    pub fn mlst_index_to_mpso(index: u16, mpso_count: u16) -> Option<u16> {
        if index < mpso_count {
            Some(index)
        } else {
            None
...
pub fn parse_pvo_file(data: &[u8]) -> Result<PvoFile> {
    ...
    let octh_sh = read_section_header(data, cursor)?;
    if &octh_sh.tag != b"OCTH" {
        return Err(Error::Parse(format!(
            "PVO expected OCTH section, got '{}'",
            String::from_utf8_lossy(&octh_sh.tag)
        )));
    }
    if octh_sh.data_length != 52 {
        return Err(Error::Parse(format!(
            "PVO OCTH data_length expected 52 (0x34), got {} (0x{:X})",
            octh_sh.data_length, octh_sh.data_length
        )));
    }
    cursor += 8;
    let header = parse_header_payload(data, cursor)?;
    cursor += octh_sh.data_length as usize;

    let octr_sh = read_section_header(data, cursor)?;
    if &octr_sh.tag != b"OCTR" {
        return Err(Error::Parse(format!(
            "PVO expected OCTR section, got '{}'",
            String::from_utf8_lossy(&octr_sh.tag)
        )));
    }
    cursor += 8;
    let octr_end = cursor + octr_sh.data_length as usize;
    if octr_end > data.len() {
        return Err(Error::Parse(format!(
            "PVO OCTR section extends past end of file \
             (section ends at 0x{octr_end:X}, file is 0x{:X} bytes)",
            data.len()
        )));
    }
    let octr_payload = &data[cursor..octr_end];
    let octr_nodes = parse_octr_nodes(octr_payload, header.total_nodes)?;
    cursor = octr_end;

    let plst_sh = read_section_header(data, cursor)?;
    if &plst_sh.tag != b"PLST" {
        return Err(Error::Parse(format!(
            "PVO expected PLST section, got '{}'",
            String::from_utf8_lossy(&plst_sh.tag)
        )));
    }
    cursor += 8;
    let plst_end = cursor + plst_sh.data_length as usize;
    if plst_end > data.len() {
        return Err(Error::Parse(format!(
            "PVO PLST section extends past EOF (0x{plst_end:X} > 0x{:X})",
            data.len()
        )));
    }
    let plst_leaves = parse_plst_leaves(&data[cursor..plst_end])?;
    cursor = plst_end;

    let mlst_sh = read_section_header(data, cursor)?;
    if &mlst_sh.tag != b"MLST" {
        return Err(Error::Parse(format!(
            "PVO expected MLST section, got '{}'",
            String::from_utf8_lossy(&mlst_sh.tag)
        )));
    }
    cursor += 8;
    let mlst_end = cursor + mlst_sh.data_length as usize;
    if mlst_end > data.len() {
        return Err(Error::Parse(format!(
            "PVO MLST section extends past end of file \
             (section ends at 0x{mlst_end:X}, file is 0x{:X} bytes)",
            data.len()
        )));
    }
    let mlst_payload = &data[cursor..mlst_end];
    let mlst_indices = parse_mlst_indices(mlst_payload)?;
    cursor = mlst_end;

    if header.mlst_polygon_count as usize != mlst_indices.len() {
        return Err(Error::Parse(format!(
    ...
    let end_sh = read_section_header(data, cursor)?;
    if &end_sh.tag != b"END " {
        return Err(Error::Parse(format!(
            "PVO expected END section, got '{}'",
            String::from_utf8_lossy(&end_sh.tag)
        )));
    }
    if end_sh.data_length != 0 {
        return Err(Error::Parse(format!(
    ...
    let actual_leaf_count = octr_nodes.iter().filter(|n| !n.is_interior()).count();
    if actual_leaf_count != header.leaf_nodes as usize {
        return Err(Error::Parse(format!(
    ...
    let plst_size = plst_sh.data_length as usize;
    let node_offsets: std::collections::HashSet<usize> =
        octr_nodes.iter().map(|n| n.byte_offset).collect();
    ...
    for node in &octr_nodes {
        for &(octant, ref_offset) in &node.child_refs {
            ...
            if !node_offsets.contains(&(ref_offset as usize)) {
                return Err(Error::Parse(format!(
                    "PVO OCTR node at 0x{:X} child octant {} references \
                     offset 0x{ref_offset:X} which is not a valid node byte_offset",
                    node.byte_offset, octant
                )));
            }
        }

        if node.leaf_ref != LEAF_REF_NONE && node.leaf_ref as usize >= plst_size {
            return Err(Error::Parse(format!(
    ...
    let mlst_count = header.mlst_polygon_count;
    for leaf in &plst_leaves {
        for entry in &leaf.entries {
            let end = entry.mlst_start as u64 + entry.count as u64;
            if end > mlst_count as u64 {
                return Err(Error::Parse(format!(
                    "PVO PLST leaf at 0x{:X} entry overflows MLST: \
                     mlst_start={} + count={} > mlst_polygon_count={}",
                    leaf.byte_offset, entry.mlst_start, entry.count, mlst_count
                )));
            }
        }
    }
...
mod tests {
    ...
    fn build_minimal_pvo() -> Vec<u8> {
        ...
        buf.extend_from_slice(&(octr.len() as u32).to_be_bytes());
        ...
        buf.extend_from_slice(&(plst.len() as u32).to_be_bytes());
        ...
        buf.extend_from_slice(&(mlst.len() as u32).to_be_bytes());
