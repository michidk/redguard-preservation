...
`frame_data[0].normal_offset` is the face data byte count (verified across all 147 animated v2.6/v2.7 files). `frame_data[0].reserved` encodes the byte gap between face data and vertex data. In some files the gap is absent and vertex data begins directly at `face_data_end`; parsers should validate the primary offset and fall back (the UESP reference uses a raw coordinate magnitude threshold of 1 090 519 040 for this check). Static v2.6/v2.7 `.3D` files use `offset_vertex_coords` directly.
