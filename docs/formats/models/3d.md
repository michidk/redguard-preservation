...
**v2.6/v2.7 animated .3DC files:** The `offset_vertex_coords` header field does **not** point to valid vertex data. Instead, vertex coordinates are located at `face_data_end + frame_data[0].reserved`, where `face_data_end` is the byte offset after the last face record, and `frame_data[0].reserved` (the first frame entry's 0x08 field) encodes the gap between face data and vertex data. This matches the UESP reference implementation (`m_EndFaceDataOffset + m_FrameDataHeader.u3`). In some files the `reserved` gap is not present and vertex data begins directly at `face_data_end`; parsers should validate the primary offset and fall back (UESP uses a raw coordinate magnitude threshold of 1 090 519 040 for this check). Static v2.6/v2.7 `.3D` files use `offset_vertex_coords` directly.
...
- [RGUnity/redguard-unity `RG3DFile.cs`](https://github.com/RGUnity/redguard-unity/blob/master/Assets/Scripts/RGFileImport/RGGFXImport/RG3DFile.cs) — 3D model importer (face/texture decoding).
